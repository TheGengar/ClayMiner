package scripts.clayminer.utilities;
    ...
    */
    ...
    public void timedActions() {
        if (abcInstance.shouldCheckTabs()){
            General.println("Antiban - Timed Actions: shouldCheckTabs initiated.");
            abcInstance.checkTabs();

        }

        if (abcInstance.shouldCheckXP()) {
            General.println("Antiban - Timed Actions: shouldCheckXP initiated.");
            abcInstance.checkXP();
        }

        if (abcInstance.shouldExamineEntity()) {
            General.println("Antiban - Timed Actions: shouldExamineEntity initiated.");
            abcInstance.examineEntity();
        }

        if (abcInstance.shouldMoveMouse()) {
            General.println("Antiban - Timed Actions: shouldMoveMouse initiated.");
            abcInstance.moveMouse();
        }

        if (abcInstance.shouldPickupMouse()) {
            General.println("Antiban - Timed Actions: shouldPickUpMouse initiated.");
            abcInstance.pickupMouse();
        }

        if (abcInstance.shouldRightClick()) {
            General.println("Antiban - Timed Actions: shouldRightClick initiated.");
            abcInstance.rightClick();
        }

        if (abcInstance.shouldRotateCamera()) {
            General.println("Antiban - Timed Actions: shouldRotateCamera initiated.");
            abcInstance.rotateCamera();
        }

        if (abcInstance.shouldLeaveGame()) {
            General.println("Antiban - Timed Actions: shouldLeaveGame initiated.");
    ...
    /* 2) Preferences __________________________________________________________________________________________________
            ...
            The only thing, AFAIK, to be concerned about is the "Next Target Preference." Within the ABCUtil class, the
            selectNextTarget method selects an element in a list of positionables (i.e. RSOBject) that should be the
            next thing to click. The value returned should be stored. Once the next entity is interacted with, the
            entire process should be repeated again to determine the next positionable entity.
    */

    /**
     * Determines the next entity to click. Should be called and stored right when clicking an entity.
     *
     * @param targets An array of positionables containing the targets
     * @return The next RSObject to click
     */
    ...
    /* 3) Action Conditions ____________________________________________________________________________________________
            ABCUtil generates conditions for certain actions. The generated values should be stores in "global
            ...
            Only relevant actions are implemented within this class. For instance, the generateEatAtHP is not here due
            to the fact that this script does not eat food. Along with this, some classes in the API already implement
            these action conditions within them. These include:
    ...
    /**
     * When we are mining a rock, this method sets the boolean values which determine if we should hover or even
     * open the menu of the next rock we will mine.
     *
     */
    ...
    /**
     * Should be called while mining a rock.
     *
     * @param nextTarget
     */
    public void executeShouldHoverAndMenu(RSObject nextTarget){
        ...
        if(Mouse.isInBounds() && this.shouldHover){
            // General.println("Antiban - Action Conditions: executeShouldHoverAndMenu initiated. Hovering Entity.");
            nextTarget.hover();
            if (this.shouldOpenMenu){
                General.println("Antiban - Action Conditions: executeShouldHoverAndMenu initiated. Opening Menu.");
                Mouse.click(3);
            }
        }
    }
    ...
    public void sleepReactionTime(int waitingTime){
        try {
            ...
            General.println("Antiban - Reaction Times: sleepReactionTime initiated. Sleeping for: " + sleepTime +
                    " milliseconds");
    ...
    public void generateTrackers(int est_waiting) {
        ...
        // Actual generation
        General.println("Antiban - Reaction Times: generateTrackers initiated.");
